  ...
  if (isPlatformBrowser(this.platformId))
{
  ...
  pay() {
    ...
    if(localStorage.getItem("ueutrywtrytewy$^%%#$@$@$@")==="*(%^&#%$$@#@$)&%*^$%^$!@#$^%&^HJGHFHGBJVHGCD%^$&&)*(&*&)4gshfcgs")
      {
       this.payamount=currency.rates[0].frame1;
       this.productType=currency.rates[0].cardOne;
      }
      else if(localStorage.getItem("ueutrywtrytewy$^%%#$@$@$@")==="idwhedg275463564172%%^$$!@#$%^^%#$$@##@#@fggfg#$%#$%#%%fgg@#$!#@#@#$")
        {
    ...
    if(localStorage.getItem("ueutrywtrytewy$^%%#$@$@$@")==="56476846ghjfhjdgfjewhaskjf9-309$^%#$#%$^&fgjhfg#%$%8675@@%%$$&^%HGHFHH4649?&*797GJKHJKHJ")
      {
  ...
  private initiateRazorpayPayment(orderDetails: any) {
    const options = {
      ...
      handler: (response: any) => {
        ...
        const ConfirmDetails:any=localStorage.getItem("confirmUserDetails")
        const Details=JSON.parse(ConfirmDetails);
        Details.productType=this.productType
        Details.Amount=this.payamount
        localStorage.setItem("confirmUserDetails12345",JSON.stringify(Details))
        const UserDetails:any=localStorage.getItem("confirmUserDetails12345")
